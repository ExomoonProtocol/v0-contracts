  ...
  describe("Functionalities", () => {
    ...
    describe("addLayer", () => {
      it("Should add a layer", async () => {
        await exomoonErc721Layered.addLayer("Background", ethers.parseEther("0.2"), 4);
      ...
      it("Should revert when not owner", async () => {
        await expect(
          exomoonErc721Layered
            .connect(accounts[1])
            .addLayer("Background", ethers.parseEther("0.2"), 4),
    ...
    describe("setLayerPrice", () => {
      ...
      it("Should revert when layer does not exist", async () => {
        await expect(exomoonErc721Layered.setLayerPrice(0, ethers.parseEther("0.3"))).to.be
          .reverted;
      });
    ...
    describe("setLayerVariations", () => {
      ...
      it("Should revert when not owner", async () => {
        await exomoonErc721Layered.addLayer("Background", ethers.parseEther("0.2"), 4);
        await expect(
          exomoonErc721Layered.connect(accounts[1]).setVariations(0, 8),
        ).to.be.revertedWithCustomError(exomoonErc721Layered, "OwnableUnauthorizedAccount");
      });
    });
    ...
    describe("setLayerVariationPrice", () => {
      it("Should set variation price", async () => {
        ...
        expect(await exomoonErc721Layered.getVariationPrice(0, 0)).to.be.equal(
          ethers.parseEther("0.3"),
        );
      });
      ...
      it("Should revert when not owner", async () => {
        ...
        await expect(
          exomoonErc721Layered
            .connect(accounts[1])
            .setLayerVariationPrice(0, 0, ethers.parseEther("0.3")),
        ).to.be.revertedWithCustomError(exomoonErc721Layered, "OwnableUnauthorizedAccount");
      });
    });
    ...
    describe("encodeLayersInfo", () => {
      ...
      it("Should encode layers info", async () => {
        ...
        for (let test of tests) {
          expect(await exomoonErc721Layered.encodeLayersInfo(test.input)).to.be.equal(
            test.expected,
          );
        }
      });
    ...
    describe("mint", () => {
      it("Should mint an item with the correct layer data", async () => {
        await exomoonErc721Layered.addLayer("Background", ethers.parseEther("0.2"), 4);
        await exomoonErc721Layered.addLayer("Character", ethers.parseEther("0.3"), 4);
        await exomoonErc721Layered.setVariations(0, 8);
